...
class FLAME(nn.Module):
    ...
    def __init__(self, config, optimize_basis=False):
        ...
        if config.flametype == 'flame23':
            loguru.logger.info("[FLAME] creating the FLAME23 Decoder")
            with open(config.flame23_model_path, 'rb') as f:
        ...
        #shapedirs = torch.cat([shapedirs[:, :, :config.n_shape], shapedirs[:, :, config.n_shape:]], 2)
        shapedirs = torch.cat([shapedirs[:, :, :config.n_shape], shapedirs[:, :, config.n_shape:config.n_shape+config.n_exp]], 2)
    ...
    def forward(self, shape_params=None, expression_params=None, rot_params=None, jaw_params=None, pose_params=None, eye_pose_params=None, neck_pose_params=None, shape_basis_delta=None, fullpose_params = None):
        """
            Input:
                shape_params: N X number of shape parameters
                expression_params: N X number of expression parameters
                jaw_params: N X number of jaw parameters (3)
                rot_params: N X number of global rotation parameters (3)
                pose_params = [rot_params; jaw_params]
            return:d
                vertices: N X V X 3
                landmarks: N X number of landmarks X 3
        """
        batch_size = shape_params.shape[0]
        if pose_params is None or (torch.sum(pose_params) == 0):
            pose_params = self.eye_pose.expand(batch_size, -1)
        if jaw_params is None or (torch.sum(jaw_params) == 0):
            jaw_params = torch.zeros([1, 3], dtype=self.dtype, requires_grad=False, device=self.neck_pose.device).expand(batch_size, -1)
        if rot_params is None or (torch.sum(rot_params) == 0):
            rot_params = torch.zeros([1, 3], dtype=self.dtype, requires_grad=False, device=self.neck_pose.device).expand(batch_size, -1)
        if eye_pose_params is None:
            eye_pose_params = self.eye_pose.expand(batch_size, -1)
        if neck_pose_params is None:
            neck_pose_params = self.neck_pose.expand(batch_size, -1)
        if expression_params is None or (torch.sum(expression_params) == 0):
        ...
        if fullpose_params is not None:
            full_pose = fullpose_params
        else:
            full_pose = torch.cat([rot_params, neck_pose_params, jaw_params, eye_pose_params], dim=1)
        #print(full_pose.shape, flush=True)
