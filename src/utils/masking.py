...
class Masking(nn.Module):
    def __init__(self, config):
        ...
        flame_mask = os.path.join(config.model.flame_mask_path)
        with open(flame_mask, 'rb') as f:
            ss = pickle.load(f, encoding='latin1')
            self.masks = Struct(**ss)

        flame_model = os.path.join(config.model.flame_model_path)
        with open(flame_model, 'rb') as f:
            ss = pickle.load(f, encoding='latin1')
    ...
    def get_triangle_forehead_mask(self):
        m = self.masks.forehead
        return self.get_triangle_mask(m)

    def get_triangle_eye_region_mask(self):
        m = self.get_mask_eye_region()
        return self.get_triangle_mask(m)

    def get_triangle_lr_eye_region_mask(self):
        m = self.get_mask_lr_eye_region()
        return self.get_triangle_mask(m)

    def get_triangle_nose_mask(self):
        m = self.get_mask_nose()
        return self.get_triangle_mask(m)
    ...
    def get_triangle_lips_mask(self):
        m = self.get_mask_lips()
        return self.get_triangle_mask(m)

    def get_triangle_ears_mask(self):
        m = self.get_mask_ears()
        return self.get_triangle_mask(m)
    ...
    def get_weights_per_triangle(self):
        ...
        mask[:, self.get_triangle_forehead_mask(), :] = self.cfg.forehead
        mask[:, self.get_triangle_eyes_mask(), :] = self.cfg.eyes
        mask[:, self.get_triangle_eye_region_mask(), :] = self.cfg.eye_region
        mask[:, self.get_triangle_lr_eye_region_mask(), :] = self.cfg.lr_eye_region
        mask[:, self.get_triangle_nose_mask(), :] = self.cfg.nose
        mask[:, self.get_triangle_lips_mask(), :] = self.cfg.lips
        mask[:, self.get_triangle_ears_mask(), :] = self.cfg.ears
